...
{ 
    ...
    $sql_query=$dbconfig->prepare("SELECT userid FROM admin_login WHERE email=?");
...
Your admin account has been created, you can login after you have activated your account and it is approved.
 
Please click this link to activate your account:
http://www.quizmaker1.000webhost.com/admin_verify.php?email='.$email.'&hash='.$hash.'
...
$password=mysqli_real_escape_string($dbconfig,$_POST['password']);
	$password=crypt($password, '$2a$07$CCSCodersUnderSiegelul$');
$sql_query=$dbconfig->prepare("SELECT  userid,email,name,activated,approved FROM admin_login WHERE email=? and password=?");
...
if($sql_query->num_rows==1)
{
	...
	elseif($row['approved']==0)
		alert("Account not approved.Please wait for the Server Admin to approve your account.");
	else{
...
<body>
  ...
  <div class="navbar-nav">
      <a class="nav-item nav-link active" href="index.php">User Login<span class="sr-only">(current)</span></a>
    </div>
</nav>
<br><br><br>
	<div class="row"><div class="col-sm-12"><h1 class="display-4 text-center">Admin Portal</h1></div></div><br>
